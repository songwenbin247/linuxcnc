...
extern "C" int sock_web_write(int sockfd, const void *buf, size_t len);
typedef enum {
...
static int sockWrite(connectionRecType *context)
{
   ...
   return sock_web_write(context->cliSock, context->outBuf, strlen(context->outBuf));
}
...
int commandSet(connectionRecType *context)
{
  ...
  pch = strtok(NULL, delims);
  if (pch == NULL) {
    return sock_web_write(context->cliSock, setNakStr, strlen(setNakStr));
    }
  strupr(pch);
    ...
    switch (lookupToken(pch)) {
      case cmdHello: 
        if (commandHello(context) == -1)
          ret = sock_web_write(context->cliSock, helloNakStr, strlen(helloNakStr));
        else ret = sock_web_write(context->cliSock, s, strlen(s));
        break;
      case cmdGet: 
      ...
      case cmdSet:
        if (!context->linked)
	  ret = sock_web_write(context->cliSock, setNakStr, strlen(setNakStr));
      ...
      case cmdShutdown:
        ...
        if(ret ==0){
          ret = sock_web_write(context->cliSock, shutdownNakStr, strlen(shutdownNakStr));
        }
	break;
      case cmdHelp:
...
extern "C" {
  ...
  }RecTypeBuf[MAX_SESSIONS];

  static libwebsock_context *ctx;

int sock_receive_cb (libwebsock_client_state *state, libwebsock_message *msg)
{
  int context_index;
  int i;
  int len;
  connectionRecType *context = NULL;

  for (i = 0; i < MAX_SESSIONS; i++){
  	if (RecTypeBuf[i].state->sockfd == state->sockfd){
  		context = RecTypeBuf[i].context;
		break;
	}
  }

  if (context == NULL) {
  	fprintf(stderr, "Error: Cannot find context for sockfd %d\n",state->sockfd);
	return -1;
  }
  ...
  len = (int)msg->payload_len;
  if (len <= 0) {
    fprintf(stderr, "linuxcncrsh: error reading from client\n");
    return -1;
  }
	printf("recv: %s\n", msg->payload);
  ...
  for (i = 0; i < len; i ++) {
      if ((msg->payload[i] != '\n') && (msg->payload[i] != '\r')) {
          context->inBuf[context_index] = msg->payload[i];
          context_index ++;
          continue;
      }
...
int sock_close_cb(libwebsock_client_state *state){
  int i;
  connectionRecType *context = NULL;
...
int sock_web_write(int sockfd, const void *buf, size_t len)
{
    int i;
    libwebsock_client_state *state = NULL;
    for (i = 0; i < MAX_SESSIONS; i++){
	    if (RecTypeBuf[i].state->sockfd == sockfd){
		state = RecTypeBuf[i].state;
		break;
	   }
    }
    if (state == NULL) {
	   fprintf(stderr, "Error: Cannot find state for sockfd %d\n", sockfd);
	   return -1;
    }
    printf("run %d\n", __LINE__);
    return libwebsock_send_text_with_length(state, (char *)buf, (int)len);
}

int sock_connect_cb(libwebsock_client_state *state)
{
    printf("new open: sockfd = %d\n", state->sockfd);
    sessions++;
    if ((maxSessions == -1) || (sessions <= maxSessions)) {
        ...
        context->commProt = 0;
        context->inBuf[0] = 0;
	RecTypeBuf[sessions - 1].state = state;
	RecTypeBuf[sessions - 1].context = context;
     }
    sock_web_write(RecTypeBuf[sessions - 1].state->sockfd, "yes", 4);
    return 0;
}

static int runWebSockets()
{
   ctx = libwebsock_init();
   if (ctx == NULL) {
   	fprintf(stderr, "Error druing libwebsock_init\n");
	return -1;
   }   
   memset(RecTypeBuf,0, sizeof(struct _RecTypeBuf) * MAX_SESSIONS);
   ctx->onopen = sock_connect_cb;
   ctx->onclose = sock_close_cb;
   ctx->onmessage = sock_receive_cb;
...
static void usage(char* pname) {
    ...
    printf("         %s [Options] [-- LinuxCNC_Options]\n"
           ...
           "         --sessions   <max sessions> (default=%d max=%d) (-1 ==> no limit) \n"
           "         --path       <path>         (default=%s)\n"
           "LinuxCNC_Options:\n"
           "          -ini        <inifile>      (default=%s)\n"
          ,pname,port,serverName,pwd,enablePWD,maxSessions,MAX_SESSIONS,defaultPath,emc_inifile
          );
}
...
int main(int argc, char *argv[])
{
    ...
    while((opt = getopt_long(argc, argv, "he:n:p:s:w:d:", longopts, NULL)) != - 1) {
      switch(opt) {
        ...
        case 'p': strncpy(port, optarg, 8); break;
        case 's': sscanf(optarg, "%d", &maxSessions); 
		  if (maxSessions > MAX_SESSIONS){
		  	printf("maxSessions cannot > %d\n", MAX_SESSIONS); exit(1);
		  }
		  break;
